...
public class Main {
	...
	private static Assembler assemble = new Assembler(); // 컨테이너 : 빈(bean) 객체를 생성, 조립후 반환 = 스프링에서 하는 일

	private static void precessNewCommand() { //회원 가입 기능
		...
		//실제적으로 회원 가입 처리
		MemberRegisterService regSvc = assemble.getMemberRegisterService(); //이미 생성된 객체를 전달 받는다.
	...
	private static void precessChangeCommand() { //비밀번호 변경 기능
		...
		ChangePasswordService changeSvc = assemble.getChangePasswordService(); //이미 생성된 객체를 전달 받는다
